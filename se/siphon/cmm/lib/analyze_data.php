	...
	/*		self::$def->pdadj = self::$def->rotaRank / self::ROTA_RANK_PASS;
		...
		//			}
			...
			if ($at12mni <= 0) {
				...
				$profitadj = pow($at12mni / self::NI_SZE_STD * .65 + self::$def->tlomr / self::RETURN_GRW_STD * .1 + self::$def->tlroer / self::RETURN_GRW_STD * .1 + self::$def->tlrocr / self::RETURN_GRW_STD * .1 + self::$def->rotaRank / self::ROTA_RANK_STD * .05, 2.3);
			}
