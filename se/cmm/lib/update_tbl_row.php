		...
		if (!@mysql_connect(DB_HOST, DB_USER, DB_PASSWORD)) {
			...
			if (!@mysql_select_db(DB_NAME)) {
				...
				if (!@mysql_query('INSERT INTO '.$tbl_name.'(tkr, mc, bps, so, ce, der, debt, cap, lyni, lyie, lypii, lyltd, lystd, lyd, lye, lycap,
					...
					tlyroe, aroeg, t12maroe, lt12maroe, slt12maroe, tlt12maroe, tlroer, afi, fe, lpba, lper, aper, lpbr, apbr, gtlp, lpgc, gtap, apgc, pc,
				...
				VALUES("'.$tkr.'", '.$def->mc.', '.$def->bps.', '.$def->so.', '.$def->ce.', '.$def->der.', '.$def->debt.', '.$def->cap.',
					...
					'.$def->slt12maroe.', '.$def->tlt12maroe.', '.$def->tlroer.', '.$def->afi.', '.$def->fe.', '.$def->lpba.', '.$def->lper.',
					'.$def->aper.',
				...
				fe=VALUES(fe), lpba=VALUES(lpba), lper=VALUES(lper), aper=VALUES(aper), lpbr=VALUES(lpbr), apbr=VALUES(apbr), gtlp=VALUES(gtlp), lpgc=VALUES(lpgc),
