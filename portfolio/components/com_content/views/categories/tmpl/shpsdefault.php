	...
	function popSmi($cats, $atcs) {	//populate sub menu items: categories and articles
		...
		if (!empty($cats)) {
			...
			foreach ($cats as $item) {
				...
				$cat->type = 'cat';
				$cat->href = str_replace('/portfolio/', '#!portfolio/#', JRoute::_(ContentHelperRoute::getCategoryRoute($item->id)));
		...
		if (!empty($atcs)) {
			...
			foreach($atcs as $atc){
				...
				$atc_sc->type = 'ctt';
				$atc_sc->href = str_replace('/portfolio/', '#!portfolio/#', JRoute::_(ContentHelperRoute::getArticleRoute($atc->slug, $atc->catid)));
